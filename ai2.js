...
// @match        *://*/*
// @exclude      *://127.0.0.1:5000/*
// @exclude      *://localhost:5000/*
    ...
    class ChameleonAIForge {
        ...
        isFrame() {
            try {
                ...
                // If window.self is strictly equal to window.top, it's the top frame.
                // If not, it's an iframe.
                // If accessing window.top throws an error (due to cross-origin),
                // the catch block will handle it, correctly identifying it as a frame.
                return window.self !== window.top;
            } catch (e) {
                // A SecurityError here means it's an iframe trying to access a cross-origin parent.
                // So, it is indeed a frame.
                return true;
            }
        }
        ...
        /**
         * Generates a Puppeteer script string based on an NLP command.
         * This script will contain a simulated NLP sequencer, executor, and reporter.
         * @param {string} nlpCommand The natural language command from the user.
         */
        async generateNLPTaskScript(nlpCommand) {
            this.ui.updateStatus('Generating Puppeteer script...', 'info');
            this.agent.trackUserActivity('generate_puppeteer_nlp_script', { command: nlpCommand });

            // This is the "NLP command-to-actionable JSON sequencer" part.
            // In a real application, this would involve an LLM API call to parse the NLP
            // into a structured, actionable sequence of steps.
            // Here, we simulate it with simple keyword matching.
            const actions = (() => {
                const lowerCmd = nlpCommand.toLowerCase();
                let generatedActions = [];

                if (lowerCmd.includes('go to amazon') && lowerCmd.includes('search for') && lowerCmd.includes('add to cart')) {
                    const productMatch = lowerCmd.match(/search for '(.+?)'/);
                    const product = productMatch ? productMatch[1] : 'product';
                    generatedActions = [
                        { "action": "goto", "url": "https://www.amazon.com", "description": "Navigate to Amazon" },
                        { "action": "type", "selector": "#twotabsearchtextbox", "value": product, "description": `Type '${product}' into search bar` },
                        { "action": "click", "selector": "#nav-search-submit-button", "description": "Click search button" },
                        { "action": "waitForNavigation", "description": "Wait for search results" },
                        { "action": "click", "selector": "div[data-component-type='s-search-result'] a.a-link-normal", "description": "Click first product link" }, // Simplified selector
                        { "action": "waitForNavigation", "description": "Wait for product page" },
                        { "action": "click", "selector": "#add-to-cart-button", "description": "Click add to cart button" },
                        { "action": "screenshot", "path": `amazon_cart_${Date.now()}.png`, "description": "Take screenshot of cart" },
                        { "action": "extract", "selector": "#nav-cart-count", "field": "cartItemCount", "description": "Extract cart item count" }
                    ];
                } else if (lowerCmd.includes('go to google') && lowerCmd.includes('search for')) {
                    const queryMatch = lowerCmd.match(/search for '(.+?)'/);
                    const query = queryMatch ? queryMatch[1] : 'Chameleon AI Forge';
                    generatedActions = [
                        { "action": "goto", "url": "https://www.google.com", "description": "Navigate to Google" },
                        { "action": "type", "selector": "textarea[name='q']", "value": query, "description": `Type '${query}' into search bar` },
                        { "action": "press", "key": "Enter", "description": "Press Enter" },
                        { "action": "waitForNavigation", "description": "Wait for search results" },
                        { "action": "extract", "selector": "h3.LC20lb.MBeuO.DKV0Md", "field": "firstResultTitle", "description": "Extract first search result title" }
                    ];
                } else if (lowerCmd.includes('visit') && lowerCmd.includes('take a screenshot')) {
                    const urlMatch = lowerCmd.match(/visit (.+?)(?: and|$)/);
                    const url = urlMatch ? urlMatch[1] : 'https://example.com';
                    generatedActions = [
                        { "action": "goto", "url": url, "description": `Navigate to ${url}` },
                        { "action": "screenshot", "path": `page_screenshot_${Date.now()}.png`, "fullPage": true, "description": "Take full page screenshot" }
                    ];
                } else if (lowerCmd.includes('fill out login form')) {
                    generatedActions = [
                        { "action": "goto", "url": "https://example.com/login", "description": "Navigate to login page (example)" },
                        { "action": "type", "selector": "input[name='username']", "value": "testuser", "description": "Type username" },
                        { "action": "type", "selector": "input[name='password']", "value": "testpass", "description": "Type password" },
                        ...
                        { "action": "screenshot", "path": `login_success_${Date.now()}.png`, "description": "Screenshot after login" }
                    ];
                } else {
                    generatedActions = [{ "action": "log", "message": `No specific sequence defined for: "${nlpCommand}". Generating a basic navigation.` }];
                    const urlMatch = nlpCommand.match(/(https?:\/\/[^\s]+)/);
                    if (urlMatch) {
                        generatedActions.push({ "action": "goto", "url": urlMatch[0], "description": "Navigate to " + urlMatch[0] });
                    } else {
                        generatedActions.push({ "action": "goto", "url": "https://www.google.com", "description": "Navigate to Google as fallback" });
                    }
                }
...
// --- NLP Command-to-Actionable JSON Sequencer (Simulated) ---
// In a real-world scenario, this function would leverage a powerful LLM API
// to interpret natural language commands and convert them into a structured
// sequence of web automation actions. For this example, it's rule-based.
function _nlpHybridSequencer(command) {
    ...
    if (lowerCmd.includes('go to amazon') && lowerCmd.includes('search for') && lowerCmd.includes('add to cart')) {
        ...
        actions = [{ "action": "log", "message": "No specific sequence defined for: \\"${command}\\". Generating a basic navigation." }];
        const urlMatch = command.match(/(https?:\/\/[^\\s]+)/);
        if (urlMatch) {
            actions.push({ "action": "goto", "url": urlMatch[0], "description": "Navigate to " + urlMatch[0] });
        } else {
            actions.push({ "action": "goto", "url": "https://www.google.com", "description": "Navigate to Google as fallback" });
        }
    }
    return actions;
}
...
// --- Executor & Reporter ---
async function performWebTask(page, actions, report) {
    ...
    for (let i = 0; i < actions.length; i++) {
        ...
        const actionLog = {
            step: i + 1,
            description: action.description || \`Performing \${action.action}\`,
            actionType: action.action,
            status: 'pending',
            error: null,
            details: {}
        };
        console.log(\`[Step \${actionLog.step}] \${actionLog.description}...\`);

        try {
            switch (action.action) {
                case 'goto':
                    await page.goto(action.url, { waitUntil: 'domcontentloaded', timeout: 30000 });
                    actionLog.details.url = action.url;
                    break;
                case 'type':
                    await page.waitForSelector(action.selector, { timeout: 10000 });
                    await page.type(action.selector, action.value);
                    actionLog.details.selector = action.selector;
                    actionLog.details.value = action.value;
                    break;
                case 'click':
                    await page.waitForSelector(action.selector, { timeout: 10000 });
                ...
                case 'press':
                    ...
                    actionLog.details.key = action.key;
                    break;
                ...
                case 'waitForTimeout':
                    ...
                    actionLog.details.duration = action.duration;
                    break;
                case 'screenshot':
                    const screenshotPath = path.join(__dirname, action.path);
                    await page.screenshot({ path: screenshotPath, fullPage: action.fullPage || false });
                    actionLog.details.path = screenshotPath;
                    actionLog.details.fullPage = action.fullPage;
                    break;
                case 'extract':
                    await page.waitForSelector(action.selector, { timeout: 10000 });
                    const extractedValue = await page.$eval(action.selector, el => el.textContent.trim());
                    report.extractedData[action.field] = extractedValue;
                    actionLog.details.selector = action.selector;
                    actionLog.details.field = action.field;
                    actionLog.details.extractedValue = extractedValue;
                    break;
                case 'evaluate':
                    await page.evaluate(action.script);
                    actionLog.details.script = action.script;
                    break;
                case 'log':
            ...
            console.log(\`[Step \${actionLog.step}] \${actionLog.description} - SUCCESS\`);
        } catch (error) {
            actionLog.status = 'failed';
            actionLog.error = error.message;
            report.status = 'failed'; // Mark overall report as failed if any step fails
            console.error(\`[Step \${actionLog.step}] \${actionLog.description} - FAILED: \${error.message}\`);
            // Optionally, break on first error or continue
            // break;
        }
        report.executedActions.push(actionLog);
    }

    if (report.status === 'in_progress') { // If no errors occurred
        report.status = 'completed';
    }
}

(async () => {
    const nlpCommand = \`${nlpCommand.replace(/`/g, '\\`')}\`; // Escape backticks in the original command
    const report = {
    ...
    try {
        browser = await puppeteer.launch({
            headless: false, // Set to true for no browser UI (faster, good for production)
            args: ['--no-sandbox', '--disable-setuid-sandbox']
        });
        const page = await browser.newPage();
        await page.setViewport({ width: 1280, height: 800 }); // Standard viewport

        console.log(\`\\n--- Starting Web Task for NLP Command: "\\\${nlpCommand}\\" ---\\n\`);

        const actions = _nlpHybridSequencer(nlpCommand);
        await performWebTask(page, actions, report);

        report.finalUrl = page.url();

    } catch (err) {
        console.error('\\n--- CRITICAL ERROR during task execution ---');
        console.error(err);
        ...
        const reportFileName = \`web_task_report_\${Date.now()}.json\`;
        try {
            fs.writeFileSync(reportFileName, JSON.stringify(report, null, 2));
            console.log(\`Report saved to: \${reportFileName}\`);
        } catch (fileErr) {
            console.error(\`Failed to save report to file: \${fileErr.message}\`);
        }
        console.log('\\n------------------------\\n');
    }
})();
            ...
            GM_setClipboard(scriptContent);
            this.ui.showNotification('Puppeteer NLP task script copied to clipboard! Paste it into a .js file and run with Node.js.', 8000, { type: 'success' });
    ...
    class UIManager {
        ...
        createWidget() {
            ...
            // Use insertAdjacentHTML for the SVG, which is often safer
            widget.insertAdjacentHTML('afterbegin', ICONS.logo);

            // Create the badge div separately
            const badge = document.createElement('div');
            badge.className = `${CONFIG.ID_PREFIX}widget-badge`;
            widget.appendChild(badge); // Append it
        ...
        createPanel() {
            ...
            // Use insertAdjacentHTML instead of innerHTML to avoid TrustedHTML issues
    ...
    class GenesisDriver {
        ...
        parseCommand(command) {
            ...
            // --- NEW: Navigate current tab ---
            const navigateCurrentTabRegex = /(?:go to|navigate to|visit) (.+?)(?: in the current tab)?(?: and|$)/;
            const navigateCurrentTabMatch = lowerCmd.match(navigateCurrentTabRegex);
            if (navigateCurrentTabMatch) {
                let url = navigateCurrentTabMatch[1].trim();
                // Basic URL validation/normalization
                if (!url.startsWith('http://') && !url.startsWith('https://')) {
                    url = 'https://' + url; // Default to HTTPS
                }
                operations.push({ type: 'navigate_current_tab', url: url });
                return operations; // This is a distinct action, usually not combined
            }
            // --- END NEW ---
            ...
            if (modifyDomMatch) {
                ...
                else if (actionDesc.startsWith('title ')) { textContent = actionDesc.replace('title ', '').replace(/^'|'$/g, ''); } // Remove quotes
                ...
                if (selector && (attribute || textContent)) {
                    operations.push({ type: 'modify_dom', selector, attribute, value, textContent });
                }
            }
            ...
            // NEW: Puppeteer NLP Task Generation
            const puppeteerNlpMatch = lowerCmd.match(/generate (?:puppeteer )?nlp task script for '(.+)'/);
            if (puppeteerNlpMatch) {
                operations.push({ type: 'generate_puppeteer_nlp_script', nlp_command: puppeteerNlpMatch[1].trim() });
                return operations; // This operation is usually standalone
            }
        ...
        async executeAgentOperation(op, context) {
            switch (op.type) {
                ...
                case 'navigate_current_tab':
                    if (op.url) {
                        this.controller.ui.showNotification(`Navigating to ${op.url}...`, 3000, { type: 'info' });
                        window.location.href = op.url;
                        // Important: This operation causes a page reload.
                        // Any subsequent operations in the same command batch will not execute
                        // on the current script instance. The script will re-initialize on the new page.
                        return null; // Indicate that no elements are returned and processing stops for this instance.
                    } else {
                        throw new Error('Navigation command missing URL.');
                    }
                // --- END NEW ---
                case 'select':
                ...
                case 'generate_ui':
                    ...
                    return null;
                case 'generate_puppeteer_nlp_script':
    ...
    class CommandPalette {
        ...
        registerDefaultCommands() {
            ...
            this.commands = [
                ...
                {
                    name: 'Generate Puppeteer NLP Task Script',
                    icon: ICONS.terminal,
                    action: async () => {
                        const nlpCommand = prompt('Enter the NLP command for the Puppeteer task (e.g., "Go to Amazon, search for laptop, add to cart"):');
                        if (nlpCommand) {
                            await c.generateNLPTaskScript(nlpCommand);
                        }
                    }
                },
                ...Object.entries(NEURAL_THEMES).map(([key, theme]) => ({
    ...
    class SessionRecorder {
        ...
        copyScript() {
            ...
            let script = `// Puppeteer script generated by Chameleon AI-Forge Session Recorder\n`;
            ...
            script += `  await page.goto('${window.location.href}', { waitUntil: 'domcontentloaded' });\n\n`;
            ...
            this.recordedEvents.forEach(event => {
                ...
                if (event.type === 'click') {
                    ...
                    if (event.checked !== undefined) { // Checkbox/radio
                        ...
                        script += `    await page.focus('${escapedSelector}');\n`; // Ensure element is focused for keyboard events
                        script += `    await page.keyboard.press('Enter'); // Simulate blur/change if needed\n`;
                    }
                } else if (event.type === 'submit') {
                    script += `    await page.$eval('${escapedSelector}', form => form.submit());\n`;
                    script += `    await page.waitForNavigation({ waitUntil: 'domcontentloaded' });\n`;
                } else if (event.type === 'keydown' || event.type === 'keyup') {
                    const modifiers = [];
